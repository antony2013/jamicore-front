        ...
        <div class="space-y-2.5">
          {#each [
            ...
            <div class="flex items-center gap-3 p-3 bg-canvas rounded-xl border border-border-subtle hover:border-brand-green/30 transition-colors">
              <div class="w-8 h-8 rounded-lg bg-brand-green/10 flex items-center justify-center shrink-0">
                <span class="text-[11px] font-bold text-brand-green">{item.step}</span>
              </div>
              <div class="flex-1">
              ...
              {#if i < 3}
                <svg class="w-4 h-4 text-brand-green/50 shrink-0" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 3v10M5 10l3 3 3-3"/></svg>
              {/if}
            </div>
          {/each}
        </div>
          ...
          <div class="product-ui-body">
            <div class="grid grid-cols-3 gap-2 mb-3">
              {#each ['Espresso', 'Cappuccino', 'Latte', 'Pastry', 'Sandwich', 'Juice'] as item, i}
                <button class="p-2.5 bg-canvas rounded-xl border border-border-subtle text-center hover:border-brand-green/40 hover:bg-brand-green/5 transition-all">
            ...
            <div class="border-t border-border-subtle pt-3">
              ...
              <button class="w-full py-2.5 bg-brand-green text-white text-[13px] font-semibold rounded-xl hover:bg-brand-green/90 transition-colors">Complete Order</button>
            </div>
          </div>
        </div>
